...
test('control buttons have accessible names', async ({ page }) => {
    await expect(page.locator('#reset-btn-panel')).toHaveAttribute('aria-label', 'Reset all selections');
    await expect(page.locator('#fullscreen-btn-panel')).toHaveAttribute('aria-label', 'Toggle fullscreen');
});
